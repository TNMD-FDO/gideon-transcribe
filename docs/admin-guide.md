...
**Prepared videos** (v1.52.0). A video is prepared by itself as its transcript lands: the picture record, the camera's stamp and the digest, in the record's lane, one video after another, and the summary and the chat are written from the result. Nobody presses anything; a summary or a chat asked for first prepares the video first, and the case page's Prepared column and Prepare them now cover the rest. The batch page and the case page say the count and the time left, and the Batch finished message waits for a batch's videos and says how they went ({prepared} in its body). As an Admin you can read the digest and every description under a recording's Details, with the versions of the templates that made them, and the **Digest** template on the Templates page is yours to refine. **The picture record, the digest, and the camera's stamp** (v1.51.0) each have a switch on the AI assistant page, greyed while Moments is Off, and every number beside them: **Picture record for summaries** with its longest span (15 seconds), shortest span, most descriptions and frames per description; **Digests** with the digest window and part cap, and the chat's descriptions near an asked time; **Read the camera's stamp**. The record is the heaviest thing a video costs the engine: one call per stretch where the picture changes, so an interview in a room is a handful and a foot pursuit is one every fifteen seconds; the summary dialog says the count before the press. The scan that cuts the record keeps its two thresholds and the gap between change points (3 seconds). The finders of v1.40.0 and their settings are gone.
