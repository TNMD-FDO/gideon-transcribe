...
**Turning the features on.** The AI assistant toggle is the master switch; Chat, Summary and Speaker suggestions each have their own. Off hides a feature everywhere; what people wrote with it is kept, not deleted. Speaker suggestions starts Off: working out who is speaking from the words alone is hard for a small model, and the office that wrote the app found the Local engine did it badly. Turn it on, try it on a few of your own recordings with Accept and Reject, and leave it on only if it earns its place; the shared engine's larger model will do better than the Local one. **Let the model think before answering** is slower and sometimes better, doubles every time limit, and gives the thinking its own room on top of the answer's budget. Leave it Off for a small Local engine: a small model thinks at length, and when its thinking uses the whole budget the answer is empty and the page says so. The first request after the engine starts is slow, twenty seconds or so, while the engine warms up; the next ones take a fraction of a second.
