...
- **Overview**: Status, Reports, Queue, Users, and the Audit log.
...
- **Help**: this guide, the user guide, and **Back to the app**.
...
Every setting has a table in the settings catalogue in the repository (`docs/spec/ADMIN-SETTINGS-CATALOGUE.md`), with its default, its limits, and its consequences. A setting that has been changed from its default carries a mark, and the default is shown beside it. Every number offers **Reset to default** beside its field while it differs from the default. This is what each page is for:
...
- **Transcription defaults**: the model, the **Diarizer** (v1.80.0: Nemotron 3 Diarization, the default, which needs no token and decides the number of voices itself; or pyannote community-1, the diarizer until v1.80.0, which needs the Hugging Face token given at install and takes the speaker-count hint; the next recording transcribed follows the change, and under Nemotron the Upload page's hint is greyed with the reason), whether speaker separation and translation are ticked by default, whether a mixed-language recording is translated without being asked, the office vocabulary that is added to every user's, and the preparation profile.
- **AI assistant**: the assistant and its parts, the engine it talks to, and its budgets: the answer caps, the time limits, the thinking allowance, the chat history, the engine window, and the Case Chat's Reading size and caps. Each starts at the value the app was built with, shown beside the field as its default, so you can try another figure against your engine and put it back with **Reset to default** beside the field, or **Reset every number on this page** under the form; both go through the tray like any change. Two worth knowing: **Engine window** is what the engine can read at once, and an office on a larger shared engine can raise it to that engine's figure so longer recordings fit; **Thinking allowance** is what "Let the model think before answering" adds to every cap, and a large model can spend all of it thinking and never begin the answer, which the page says. An installation with no engine leaves the assistant off and everything else works.
...
- **Appearance**: the **What the chat is called** (v1.67.0, Appearance): the chat's name on every page, shipped as Gideon; empty falls back to Chat. The name is what people see; the word in the settings, the file names and the audit log stays Chat. It is the app's chat, not the GIDEON engine the app talks to. **Office name**, shown under the logo on the sign-in page and on the cover of every Word export, the **Logo on Word exports** toggle, and **Marking on exports** (v1.90.0), the line at the head of every page of every Word export, shipped as "Privileged and confidential. Attorney work product."; empty prints no marking. The same page holds the **office logo** itself, uploaded and removed at once rather than through the tray: a PNG or JPEG up to 2 MB, a wide mark reads best. It appears large on the sign-in page as soon as it is uploaded, and at the head of every export's cover once the toggle is on. It is kept with the app's own data, so a backup carries it, and it never leaves the server.
- **Incidents**: the **Incidents** switch (greyed while Folder management is off; off hides the Incidents tab, the incident pages and every All cameras button, and keeps every incident), **Incidents proposed** (the case page's offer when videos overlap on their clocks), **Cameras on the wall** (six; how many videos a workstation plays at once), **Incident layout** (Focus; the layout a person starts with before choosing from the page's Layout menu, which the browser remembers) and **Most cameras in an incident** (twenty), **Stamp reads early** (read each camera's burned-in clock as its playback copy lands, two small engine calls per video by day, rather than with the night's vision work; needs Vision's Read the camera's stamp), and **Match by sound** (offer to place a camera without a clock by its sound, on the media worker, with no engine call). Under **The assistant** (v1.61.0, greyed while the AI assistant is off as well): **Case chat knows the incidents** (the case chat is told each synced camera's start by the incident clock), **Assistant proposes events** with its **answer cap** and **time limit** (Propose events on the Chronology tab, one engine call per synced camera, nothing applied until a person accepts), and **Incident memo** with its **answer cap**, the **Facts sheet answer cap** and its **time limit** (the Memo tab; since v1.91.0 the memo is two engine calls, the facts sheet drawn from the cameras' digests merged onto the incident clock and then the memo written from the sheet alone, with a continuation call when the memo is cut at its cap). The wording is the **Proposed events**, **Memo facts sheet** and **Incident memo** templates on the Templates page. The audit log writes one AI assistant call row per run (features `incident_events` and `incident_memo`, the memo's row counting its calls), Events proposed, Event added (source assistant) and Event dismissed, and Incident memo exported, never a word of a proposal or the memo. **Incident chat**, with its **answer cap** and **time limit** (v1.67.0, under The assistant): Ask Gideon on the incident page, a chat grounded in the incident record and the chronology; Off hides the button and keeps every conversation. **About this office and case**, **Proposed events window**, **Second look** and **Watch phrases** (v1.64.0, under The assistant): what the office tells the proposer about itself and its cases, given to every Propose events run as context; how much of a camera one engine call reads (ten minutes shipped); whether each stretch gets a second call that reads it again against what was proposed (On; it doubles the calls); and the words the office always wants an event for, one per line, shipped with a default list to edit to your practice, looked for by the app itself on every run before the engine is asked, in each camera's spoken words and in what its picture showed (the Digest's lines from the vision model), whole words and any case, so a hit is never lost to the engine's judgement. An empty list loses nothing of the finder. **Incident clips** (on; greyed while Incidents or Clips available is off), which offers Clip this event on the Chronology tab: one file cut on the media worker from an event's cameras over its span, the incident clock and the camera ids burned in, listed with the case's other clips. The Clips page's own Longest clip applies; the render's time limit is the clip's length plus five minutes plus a minute per camera, and incident clips render one at a time. Off hides the button and keeps every clip made. The audit log's Clip created row carries the count of cameras and the layout, never a word of the event.
...
The **Documents** page of the settings turns the police report beside the cameras on or off and sets its limits: **Largest document** (pages; a bigger PDF is refused at upload with the reason), **Documents per incident or recording**, and **Read scans with OCR**. A scan is a PDF of pictures; with OCR on, the Tesseract engine in the app image reads its words on the CPU at upload and marks the page "read by OCR", and a page it read poorly is marked so and its words are not to be trusted. With OCR off a scan is kept and shown and its words are not read. **Reading ceiling** (paragraphs) is the most Gideon reads whole with a question; past it, only the paragraphs whose words match the question are read, and the answer says so. A thirty-page report is about 150 paragraphs, so the default of 400 reads a normal report whole and trims a stack of them. **Compare with the report** turns the comparison on (it needs the assistant); **Comparison answer cap** and **Comparison time limit** are one window's figures, and a cap hit is counted on the comparison's state line; **Omissions at most** (v1.91.0) is how many events the report leaves out one comparison may list, the ones that matter most first. Each finding carries what it rests on (the picture, the person's words, an officer's words, nothing), and one the model marked agrees on an officer's words alone is kept as Not on camera with the why saying so (v1.90.1). The comparison reads the report in windows of eight pages, one engine call each, plus one call for what the report leaves out, so a sixty-page report is about nine calls; the audit log shows one AI assistant call row for the run with the feature compare_report and the number of calls, and one Comparison run row with the counts. The Comparison template on the Templates page is its instructions. A comparison that comes back with fewer findings than expected says on its state line how many answers the app could not read and how many findings it dropped, by reason (a paragraph the report does not have, a time not on the incident's clock, a mark that is not one of the four); the same two counts are on the Comparison run row of the audit log. Nothing about documents needs configuring at install and nothing leaves the building: the reader, the renderer and the OCR engine are in the app image, and the pages are kept under the case's folder and go with it. From v1.74.0 a Clip created row for an incident clip also says which way the clip was started (from the button, from the strip, from a line, from an event); the incident page's Clip button, clip track and Clips lane follow the Incident clips setting and stay greyed with the reason when it is off. The panel width a person drags on the incident page is kept in their own browser, never on the server.
...
**The microphone check.** The New recording page lists the computer's microphones with a live level bar before Record, remembers the choice per computer, and warns when nothing reaches the app for a few seconds, on the page and in the first seconds of a recording. The usual cause is a Remote Desktop session having left Windows on "Remote Audio"; the page names it when it sees it. The fix is in Windows Sound settings, or the chooser.
...
The AI assistant, Summary, Chat, Speaker suggestions and Moments, talks to a language-model engine through the engine's own API. The app never runs a model itself. Which engine is yours to say, on the panel's **AI assistant** settings page:
...
**The engine's window** (v1.78.0). The app asks the engine what it can read at once (vLLM reports it as `max_model_len` on the served model) with the same once-a-minute check that asks whether the engine answers, and every fit and every bar in the app uses that figure. The **Engine window** setting on the AI assistant page is the fallback for an engine that does not say; while the engine reports its own, the row greys and shows the engine's figure. The Status page's AI assistant block says which is in force ("read from the engine at 11:20" or "from the setting; the engine did not say") and how many readings were refused as too long today and this week. When the office moves to a model with a larger window, nothing here needs editing: the bars on every incident page follow within a minute.
...
**Turning the features on.** The AI assistant toggle is the master switch; Chat, Summary, Speaker suggestions and **Chat across cases** each have their own. Chat across cases is the Case Chat, the Chat tab on every case page that answers from every transcript in the case; it is the heaviest thing the assistant does, up to fifteen minutes and twenty engine calls for one question on a large case, so it has its own switch, independent of the viewer's Chat toggle, and its own ceiling on the Limits page, **Case chat: most hours of talk per question** (120 hours by default, 6 to 600). A question over the ceiling refuses and tells the person the figures. Its instructions are the **Case chat** template on the Templates page. Off hides a feature everywhere; what people wrote with it is kept, not deleted. Speaker suggestions starts Off: working out who is speaking from the words alone is hard for a small model, and the office that wrote the app found the Local engine did it badly. Turn it on, try it on a few of your own recordings with Accept and Reject, and leave it on only if it earns its place; the shared engine's larger model will do better than the Local one. Since v1.11.0 the app does the finding itself first: **Speaker suggestion method**, greyed while Speaker suggestions is Off, names how. Its one method today is "evidence": the app picks out the self-introductions and forms of address in the transcript, hands them to the engine with the case's people and your Speaker roles, and keeps a name only when the words back it, so the engine offers a role or nothing rather than a guess. Better methods, from more capable models or new pipeline tools, join that list as they are built, and you choose among them here; the Speaker suggestions toggle turns the whole feature off whichever method is chosen. **Let the model think before answering** is slower and sometimes better, doubles every time limit, and gives the thinking its own room on top of the answer's budget. Leave it Off for a small Local engine: a small model thinks at length, and when its thinking uses the whole budget the answer is empty and the page says so. The first request after the engine starts is slow, twenty seconds or so, while the engine warms up; the next ones take a fraction of a second. **Moments** (Phase 4) starts Off and needs an engine that can look at video: the shared engine's Qwen3.8 model can, and so can the Local engine's Qwen3.5, while an engine that takes text only answers "This engine cannot look at video" and the Moment fails honestly. With it On, a video recording's viewer gains a Moments tab and a camera button on every line; the app cuts a short clip of the playback copy around the chosen time on the worker, shows it to the engine with the words spoken in it, keeps the description and deletes the clip. **Moments in answers** hands those descriptions to Summary and Chat as labelled camera lines. Five numbers shape a Moment, each with Reset to default: **Moment clip length** (10 s, half before and half after), **Moment frames a second** (2), **Moment frame height** (360 pixels; each 640 by 360 frame costs the engine about 150 tokens, so a ten-second clip is about 3,000), **Moment answer cap** (250 tokens) and **Moment time limit** (120 s, never doubled, since a Moment never lets the model think). **Moment style** chooses brief descriptions (one to three sentences leading with the thing pointed at, the default) or full ones with the setting and the seconds. A question about a moment ("is that a gun on the seat?") is answered from a few still frames at the camera's own detail rather than the small clip: **Look closer frames** (3, spread a second either side) at **Look closer frame height** (720 pixels; about 1,200 tokens a frame), in a fixed shape of what is visible, what it is consistent with, and what cannot be told. **Find moments** has two finders, each with a switch. **From the words** (On) reads the whole transcript once with the engine and suggests the lines where the picture would add a fact, each with a reason; **Most suggested moments** (12) and **Least sure suggestion kept** (medium) tune how many and how sure, and it has its own answer cap and time limit. **From the picture and sound** (Off until you have judged it) scans the video without the engine on the media worker, a few minutes for a long recording: **Picture change threshold** (40 percent; a body camera that swings about needs a higher figure), **Loudness rise** (12 dB above the recording's usual level), and **Gap between scanned moments** (15 s, so a struggle is one suggestion). The finder's instructions are the **Find moments** template on the Templates page. The word list that marked lines in v1.38.0 is gone: it flagged ordinary talk. **Describe the whole recording** makes a Moment every **Describe at intervals: every** seconds (60), at most **Describe at intervals: at most** (40) spread evenly, one engine call each in a single lane so a long recording never takes the assistant's four lanes from everyone else. **Summaries describe the moments first** (On since v1.43.0) sets whether **Summarise this video** starts with its tick, Look at the picture first, ticked; it starts ticked only while something is left to describe and fewer moments are described than **Enough moments for a video summary** (10; 0 means always). Ticked, a summary runs the intervals before it writes and draws on what was seen, and its Word export ends with What the camera showed. On a shared engine that is forty calls of a few thousand tokens for an hour of video, a minute or two of the engine's time. A video without a recording type gets the **Video summary** template, which writes from the words and the moments together and has a part Seen but not said; a body camera recording keeps its Body camera summary, which gained the same two parts (Reset to default puts them into a template you had edited). Whenever moments are handed to Summary or Chat the app adds fixed camera rules after your templates: the two sources kept apart in every sentence, neither winning, a person named only from the words, nothing inferred from the picture, only the listed times looked at, and camera facts told briefly; the summary is also told how many camera lines to draw on for its length (5, 12, or 30). Try it on a few of your own body-camera recordings and read two or three summaries before leaving the defaults on; a description is a lead to check against the picture, not a finding.

**Templates.** The Settings group of the rail has a **Templates** page: the twelve prompt templates (the Ground rules every call starts from; Chat; Speaker suggestions; Case chat; Moment; Digest; Speaker check; Proposed events; Incident memo; Memo facts sheet; Incident chat; Comparison) and the Summary templates. Each is plain text you can edit, with **Reset to default** and a version that rises by one on every save; changes apply at once, outside the tray, and the audit log records the template and its new version, never the words. A Summary template has a name, a one-line description, the **recording types it is for**, and Enabled and Default flags; exactly one of the Enabled is the Default, users choose a template only when two or more are Enabled, and the shipped templates cannot be deleted. The app ships nine: Standard summary, the Video summary (for a video without a type, when Moments reach answers), and one each for Jail call, Body camera, Interview, Phone call, Hearing, Dictation and Meeting, each ticked for its type and Enabled from the first day; read their wording here and edit it in the office's own voice, or Reset to default to put the shipped words back. The Standard and Video summaries are written as a memo a member of staff hands to an attorney (v1.53.0); the Body camera summary is the investigator's memo (v1.90.0), with points for counsel as questions. A shipped template you have not edited follows the wording of each new release by itself, its version rising as if reset; one you have edited keeps your words, and the page marks it "shipped wording changed" so you can Reset if you want the new wording. A recording in a case with a type gets that type's template preselected in the viewer, listed first, with the others still on offer; a recording without a type, or outside a case, gets the Default. **Add template** adds another shape of summary, ticked for whichever types it is for (none means any), and the viewer offers it from that moment. The ticks follow the Recording types list under Cases settings; a type you remove from the list stays on the templates that had it, marked "no longer listed", until you untick it. The same page holds the **starter questions**: two lists, one for a recording's chat and one for a case's, one question per line, that an empty chat offers as one-click chips. They start empty, so nothing is offered until you add some; good ones are the questions your own staff actually ask, in their own words.

**Vision** (v1.54.0; Prepared videos in v1.52.0). The Settings group has a **Vision** page holding every picture setting under its group: the switch **Vision**; **Vision runs**, which is the decision that matters on a shared engine: *as each transcript lands* (shipped: the picture is described the moment a transcript lands, session recordings too), *overnight* (recordings in a case wait for the window **Overnight from** and **Overnight until**, two clock times in the server's zone, oldest first, one at a time; a video not reached is first the next night), or *only when asked* (nothing runs until you allow it for a case); **Enrich with vision starts ticked**, the upload page tick's starting position; the descriptions, the scan, the record, the digest, the chat and the camera stamp as before, under their new names (Moments is Vision, Moments in answers is Descriptions reach summaries and chat, the Moment prefix is Description); and **Exports carry what the camera showed**, Off to keep the descriptions inside the app, which governs a summary's export and a clip's captions, and **Transcript exports carry what the camera showed**, Off as shipped since v1.57.0 so a transcript export is the words. At the top of the page sit the **requests**: anyone with a case may Ask for it now, and you Allow (the work starts at once, the asker is mailed) or Decline (with a line the asker sees on the case page); the rail shows how many wait, and every Admin with an address is mailed when one arrives. On any case page you have **Enrich now**. Only an Admin can start vision work by day; a summary or a chat question never does under a schedule. The Email page gains three templates, Vision done, Vision requested and Vision allowed, and the Batch finished body's {prepared} line says what will happen and that a second message follows. The audit log writes Video enriched, Vision queued, Vision requested, Vision allowed and Vision declined, never a word of a description or of the line of why.

**Speaker check** (v1.55.0). The Settings group has a **Speakers** page for the check the assistant makes of a transcript's speaker labels: after a transcript lands with its speakers told apart, the engine reads it in windows of about ten minutes and proposes the lines whose words show they belong to another speaker, and a person accepts or dismisses each on the Speakers page; nothing moves by itself, no speaker is added or merged, and no word changes. The switch **Speaker check** ships Off: turn it on after seeing it do well on your own recordings, as with Speaker suggestions. **Speaker check runs** is *as each transcript lands* or *overnight*, when a check queued by day waits for the Vision page's window to open; a press of Check the speakers on the page runs at once either way. **Speaker check window**, **answer cap** and **time limit** size one engine call. The wording is the **Speaker check** template on the Templates page. The audit log writes Speaker check queued, one AI assistant call row per run with the windows and the corrections found, and Speaker correction accepted or dismissed, never a word of the transcript.

**When it fails**, the app says one of seven things and records which: not available (the engine cannot be reached), took too long, refused the connection (the token is wrong or missing: "Ask IT" means you), too long a transcript, an answer the app could not read, an engine that cannot look at video (a Moment only), or a problem. It never records what was asked or answered.
