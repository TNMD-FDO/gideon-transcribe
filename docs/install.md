...
- **Memory, cores and disk.** The app itself is light on the host: about 4 cores and 16 GB of memory serve the app, its workers, the web server and the database on a server with no card, and the diarizer beside the transcription service wants about 4 GB more. More cores make media work faster (the appendix's two media keys are how you share them); the card does the rest. These are the app's own estimates, not a measured floor, and the server the office that wrote the app runs is a much larger machine shared with other work, so it says nothing about the least that serves. The root disk holds Docker's images and build cache: keep 100 GB free there, and the upgrade refuses to build an image with under 40 GB.
