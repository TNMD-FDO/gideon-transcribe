...
| Watch phrases | text, one phrase per line, up to 200; greyed while Incidents is Off; resettable to the shipped list | the shipped list (weapons, force and injury, commands and rights, consent and search, admissions and threats, drugs, identification) | 7 (chapter 2, v1.64.0) | Words and phrases the office always wants an event for. On every Propose events run the app itself searches each synced camera's transcript (and its Digest's picture lines) for them, whole words, any case, and proposes an event at every line that carries one, with the source Watch phrase, before and whether or not the engine is asked. | The next run. |
