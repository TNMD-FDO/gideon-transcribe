...
| What counts as an event | text up to 2,000 characters; greyed while Assistant proposes events is Off | empty | 7 (chapter 2, v1.64.0) | The office's own instructions to the proposer, given after the Proposed events template on every run as "The office adds:", so an office steers the proposals without editing the template. | The next run. |
