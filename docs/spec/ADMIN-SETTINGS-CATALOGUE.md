...
| Summaries describe the moments first | On or Off | On (Off in v1.41.0) | 4 (v1.41.0; the rule from v1.43.0, simplified v1.51.0) | Whether Summarise this video starts with its tick, Look at the picture first, ticked, while anything is left to describe. Ticked, the summary makes the picture record before it writes; the cost is one call per span. Greyed while Moments is Off. | The next summary dialog opened. |
...
| Digest window | tokens, 4,000 to 40,000 | 12,000 | 4 (v1.51.0) | How much of the transcript one digest call condenses, about twenty minutes of talk at the shipped figure; one call per window. Greyed while Moments is Off. | The next summary; only the changed windows are remade. |
