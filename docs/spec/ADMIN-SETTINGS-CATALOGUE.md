...
| Interpreter (Features page; key `interpreter`; built in v1.33.0, text only) | On or Off; greyed while Live recording is Off; the page is absent while the AI assistant has no engine | Off | 3 | People may hold an interpreted Session from the New recording page: heard by the service, translated by the engine, kept as a Recording in both languages. | At once. Off hides the page; a Session in progress finishes. |
| Languages offered (Interpreter page; key `interpreter_languages`) | one Whisper code per line | `es` | 3 | The visitor languages staff may choose; offered only after a bilingual reader has checked a sample. | The next Session. |
| Turn-taking (Interpreter page; key `interpreter_turn_taking`) | `tap`, `hold` | `tap` | 3 | Tap: one button is always lit for whose Turn it is and a Turn ends at a pause; Hold: a Turn is the time a button is held. | The next Session. |
| Readback (Interpreter page; key `interpreter_readback`) | On or Off | On | 3 | Each person sees their own words as heard beside the translation. | The next Session. |
| Quick phrases (Interpreter page; key `interpreter_phrases`) | one line per row | the six shipped | 3 | The Staff side's ready lines. | The next Session. |
| The notice (Interpreter page; key `interpreter_notice`) | text | the shipped wording | 3 | Shown at the top of the Session page, in English and in Spanish for Spanish. | The next Session. |
| Spoken output, When to speak, Voice per language, Speaking speed (Interpreter page; **not yet built**) | see Phase 3 chapter 3's table | Speak to the visitor only; As soon as ready; the first installed Voice; Normal | 3 | How a Session speaks. | At once; a Session in progress keeps the settings it started with. |
