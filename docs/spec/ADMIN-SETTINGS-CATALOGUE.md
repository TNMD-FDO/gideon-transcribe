...
An Incidents page in the Settings group between Vision and Speakers, specified in `docs/spec/SPEC-PHASE-6.md`. Every row but the first is greyed while Incidents is Off; Incidents itself is greyed while Folder management is Off, since an Incident lives inside a Case.

| Setting | Type | Default | Phase | What it does | When changed |
|---|---|---|---|---|---|
| Incidents | On or Off; greyed while Folder management is Off | On | 6 | A case's videos that ran at the same time become an Incident and play in step on a page of their own, each camera placed on the clock its picture carries. | At once. Off hides the Incidents strip and New incident on every case page, the Incident column, every Incident page and every All cameras button; keeps every Incident, placement, and (later chapters) Event and memo. |
...
- From Phase 6 chapter 1 (2026-09-17, written for the build), an Incidents page in the Settings group between Vision and Speakers: Incidents, Incidents proposed, Cameras on the wall, Most cameras in an incident, Stamp reads early, Match by sound; and Read the camera's stamp's description says the stamp reads as the playback copy lands under that switch.
