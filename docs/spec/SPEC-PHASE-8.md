...
- **2026-09-26, v1.90.1.** Chapter 4, from the second reading of the outputs on the office's copy after v1.90.0: the re-shipped template alone had not stopped the model marking "agrees" on who was driving from "I'm assuming he's the driver" (fifteen agreements, none differing, thirty-nine not in the report, one window cut at the cap). Each finding now carries its basis (picture, person, officer, none) in the JSON, and the app keeps an agrees resting on an officer's words alone, or on nothing, as not on camera with the why saying so; the four marks and the page order are unchanged. The pass for what the report leaves out asks only for what the report should have told (a plea, a denial, a request, a command, force or injury, a search or what was found, a question and its answer, ownership, a camera muted), fifteen at most, and says radio talk, the scene described and an officer's remarks to another are not findings. The Comparison answer cap's shipped default is 6,000 tokens; one five-page report's only window had been cut at 3,000.
- **2026-09-25, v1.90.0.** Chapter 4's comparison template is re-shipped from the maintainer's walk of the outputs: the record agrees with a claim only when the words or the picture show the thing itself, an officer's say-so on camera being not on camera with the say-so quoted; a denial by the person stopped is a difference; a paragraph that tells part of the stop and leaves out what the cameras hold there (a denial, a request for a lawyer, an injury, ownership, a search, a question, a camera muted) differs, the why saying what it leaves out; the findings spent on the defence, agreement kept to the claims that decide the case. On the office's copy the comparison had marked "agrees" on who was driving from an officer's "I'm assuming he's the driver", thirteen agreements to one difference where the incident chat had found four differences and ten omissions. The four marks, the paragraph-order sort and the chronology pass for what the report leaves out are unchanged.
