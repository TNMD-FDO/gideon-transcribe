...
- **Edit** on the Chronology tab opens the same box on that Event, with **Remove**. Changing the time moves it on the lane; changing the cameras changes Seen on. An Event whose source camera has left the Incident keeps its text and reads "a camera no longer in the incident" for its source. (Phase 8 chapter 11: a note event's time is its line's; Edit changes its words, which are the note's, its cameras and its mark.)
- **Where an Event goes.** With the Incident: deleting the Incident removes its Events; a case in the Recycle bin takes them and brings them back; a recording deleted leaves its Events standing without their camera. Adding, changing or removing an Event counts as the case's activity. (Phase 8 chapter 11: a fifth source, note; a note event goes with its line, its camera's place in the Incident, and the note.)
...
- The strip's last lane, drawn from chapter 1's empty one: a mark at each Event's time, or a short bar from its time to its end, with the first few words of its text beside it; a person's Event and a quoted one in the accent colour, and chapter 3's proposed ones in the working colour. A press on a mark seeks every camera there; hovering shows the whole line. (Amended by Phase 8 chapter 11: no words beside a mark at any zoom; a press seeks and opens the Event card, hovering shows it light.) When two labels would overlap at the zoom in hand the later one is drawn as a mark alone, and the Zoom buttons of the strip spread them.

### The exports

- **Export** in the head offers three: **Chronology to Word**, **Chronology as a spreadsheet** (.csv), and **The strip as a picture** (.png). Each writes the audit row Chronology exported with the format, never a word of an Event. (Amended by Phase 8 chapter 12, v1.85.0: two exports, Word and the spreadsheet; the picture is gone.)
- **The Word document**, in the app's export shape (the office's name and logo on the cover as the other exports carry them): the title "Chronology: <incident>", the case, the Incident clock's date, the cameras with how each was placed (the pills as printed), the strip as a picture, then the table: number, time (the time of day), event, source, seen on (amended by Phase 8 chapter 12, v1.85.0: the chronology figure in place of the picture and the table, the band of the cameras' spans and the events down the page in spells, each numbered in its camera's colour with its time, its line and, under it, the note, the why and the source). Under the table two lines: that an event marked Words or Camera was taken from a transcript or from a model's description of the picture and accepted by a person, and that a quoted transcript may have been corrected since. The AI notice prints only when any Event was proposed by the assistant (chapter 3); the transcription notice does not print, since the document is not a transcript.
...
- **The picture**: the strip as the page draws it, at twice the screen's resolution, with the ruler, a lane per camera with its id and bar, and the Events lane with its marks and labels, in the light palette whatever the person's theme, so it reads on paper. The page draws it on a canvas from the same rows as the strip and hands it to the export; the Word document embeds it, and The strip as a picture saves it alone. Nothing of it is stored. (Reversed by Phase 8 chapter 12, v1.85.0: the page draws nothing for an export; the server draws the chronology figure, and the picture export is gone.)
...
- **2026-09-26, v1.91.1.** From the first two-pass memo on the office's copy: the sheet was cut at 8,000 tokens with four lists unwritten, so the Facts sheet answer cap ships at 12,000 and the sheet's shape puts the timeline last, so a cut costs the timeline's end (which the app can see) and not a whole list; the sheet carried the machine's speaker labels ("Speaker 4 asked") and the memo copied them, so the app scrubs a label or "unidentified speaker" out of the sheet as "a voice on <camera>" (a quote is left as it is), and the sheet's template lists an officer only when he did something; a continuation had joined "Aldridge" and "answered" into one word, so a cut memo is trimmed back to its last whole sentence and the model told to begin with the sentence that was cut.
- **2026-09-26, v1.91.0, ADR 0016.** Chapter 3's Incident memo is written in two passes. The first, the **Facts sheet**, reads the record and the chronology as the memo did and answers as data under a schema: people, a timeline of the moments from the first camera's start to the conclusion (each with its time, camera, one sentence, a quote word for word, who said it, the event it rests on), rights, questions before rights, searches and force, statements, gaps, the outcome; its template, Memo facts sheet, is editable on the Templates page; its cap is the Facts sheet answer cap (8,000 tokens). The app checks the sheet (every time inside the span, every quote looked for in the record word for word, the timeline reaching the end) and marks rather than drops. The second pass writes the memo from the sheet alone with the Incident memo template, re-shipped to say so; the record is never in front of the writer. The sheet is kept with the memo, shown on the Memo tab under a fold with its times as citations, printed in Memo to Word between the memo and the Chronology, and **Rewrite from the sheet** runs the second pass alone while the chronology and cameras are as they were. The audit row counts the calls and the moments. Migration 0063. Why: on the office's copy the model wrote the memo by copying the record's lines, whatever the wording said (v1.90.0 and v1.90.1); the ADR has the figures.
- **2026-09-25, v1.89.0.** From the walk of v1.88.0: the offer's suggested name carries the first checked camera's id beside its date ("11/06/2025, BWL7-004997"), so an incident is told from the Made column's dates.

- **2026-09-24 (Phase 8 chapter 11, v1.81.0).** Events: a fifth source, note, for the office's note on a synced camera's line, which is an Event by itself. The Events lane: a mark alone per Event, the Event card for its words.
