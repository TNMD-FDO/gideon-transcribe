...
6. **A page of its own.** The Incident page is reached from the case page and left by Done, in the Speakers page's shape. The case page carries only the doorway: the strip of the case's Incidents under its name.
...
- **Placed by hand**: dragged on the strip, or nudged with the buttons (a second and a tenth of a second either way) while both play with the sound from one, or typed as a clock time. A camera placed from its clock and then nudged becomes Placed by hand.
- **Not placed**: none of the above has happened. The camera is on the Cameras tab with **Place**, on the strip as a thin bar at its file time when it has one and at the Incident's start when it has not, and never on the Wall.
...
- **The Wall.** Up to **Cameras on the wall** (six shipped) placed cameras, in a grid the build chooses for one to nine (three across at six), in the order of their start on the clock unless a person reorders them. Each tile: the camera id from its stamp when there is one, else the recording's title, with its colour dot; the picture; and under it the line being spoken on that camera, in that camera's Speaker colours, and, on a video enriched with vision, the camera line whose span holds the moment ("Camera: at the driver's window; a hand on the door frame"). A tile whose camera has not started at the clock's moment shows "Starts in 0:47" over a dark picture; one whose camera has ended shows "Ended at 21:57:30"; the tile stays, so the Wall does not rearrange itself while playing. The remaining placed cameras are listed under the Wall as parked tiles with **Swap in**, which takes a camera off the Wall (the one the person picks, or the last) and puts this one on; the swap is remembered for the Incident.
...
- **Phase 2, the Cases chapter, The Case page**: the Incidents strip under the case name, New incident beside Add recordings, and the Clock in the picture and Incident columns on the recordings list. The four tabs stay.
...
- **Incidents** (On or Off, On shipped; greyed while Folder management is Off). Off hides the strip, New incident, the Incident column, the Incident pages and every All cameras; keeps everything.
