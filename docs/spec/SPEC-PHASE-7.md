...
- **2026-09-26, v1.90.1.** Chapter 3, from the second reading of the outputs on the office's copy after v1.90.0: the twelve-camera memo hit its 4,000-token cap at 2,100 words with five parts unwritten, listed thirteen officers one a camera, and copied the chronology's lines with their speaker labels and watch phrases. A memo cut at the cap now goes on from where it stopped: up to two more calls, each shown the memo so far and told to continue without repeating, the pieces stitched where the cut fell; "The memo was cut short" is kept for a memo still cut after the third call, and the audit row counts the calls. The template is re-shipped: People lists only those who matter; What happened is at most six phases of prose, two hundred words each; the chronology's and the record's lines are notes to write from, never sentences to copy, no sentence beginning with a time and a speaker's label, and no watch phrase mentioned as such.
