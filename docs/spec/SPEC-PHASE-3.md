...
Written 2026-09-07 at the maintainer's ask, after the Record page, Dictation, and Send to were built and in use. **Built as a first, text-only slice in v1.33.0 to v1.34.0 and withdrawn in v1.35.0 the same day, at the maintainer's decision:** the office is not ready to take it on and does not expect it to be used much. The chapter stands as the design, deferred (chapter 5); the code is in the repository's history at tag `v1.34.0`, and its two research notes (`docs/research/interpreter-languages.md`, `docs/research/piper-voices.md`) stay. What the first live sessions taught is folded into the amendments below, so a later build starts from the corrected design, not the chapter's first guess.
...
- **The Interpreter as a whole** (chapter 3): **deferred** on 2026-09-07 by the maintainer after its first live sessions, "I'm not ready to tackle this just yet and I don't believe it will be used much at our office". Built and withdrawn the same day (v1.33.0 to v1.35.0); the chapter, its amendments, and its research notes remain for the day it is wanted.
...
- **From the maintainer, on the v1.30.0 build, to the Upload page:** "speaker diarization can be spotty enough that for now I never want it selected by default. So I don't want to give them a bunch of mode choices"; the Start page's upload door should say a batch is fine; "one simple clean page"; and "a clear indicator that diarization isn't perfect when they decide to turn it on themselves". Built in v1.30.1: the What are these? cards and their presets are gone; the page asks only where the files go (when Folder management is on) and then takes the files; Diarize is "Tell the speakers apart", off unless ticked, with an amber notice shown while it is on saying it is not always right and the labels are guesses until checked; step 2 is "Settings (optional)"; the Start page's card reads "One recording or a whole batch".
  - **The fast lane, built in v1.31.0** ahead of step five: the `whisperx-fast` Compose service under the `fast` profile, its own state folder, the same model; `./transcribe fast-lane on|off` and an install question; `WHISPERX_FAST_URL` in `.env`; a Run remembers its lane (migration 0030); a Live recording's Runs go to the lane when it answers and to the batch service at the live priority when it does not; the Status page's Fast lane card; `./transcribe check`. Chapter 3's "smaller model" was corrected to the same model at the build, for principle 2.
  - **Step five, transcription during the recording, built in v1.32.0** as the chapter describes, with these decisions left to the build: a stretch is five minutes of recorded time and a pause closes one when it holds at least twenty seconds (a shorter one runs on); the page tells the server a stretch has closed once the piece holding its end has been sent, and the media worker cuts it from the sidecar's growing file (or the original, once taken), prepared and loudness-normalised on its own, checking the cut is the length the page said and trying again a few times while the last pieces arrive; each stretch is a Run of the recording's one open Job (`Job.open`, `Run.stretch`, `Run.offset_seconds`, `Run.seconds`, migration 0031), sent at priority 100, with a recording that has ended at 90 and uploads at 0; the Job is closed at Stop with the tail as the last stretch, and a stretch that failed while recording is cut again then. **Speakers across stretches** differ from the chapter's "one pass over the whole audio": the service has no diarize-only request, so each stretch is diarized on its own with `return_speaker_embeddings`, and the labels are matched across a Side's stretches by cosine similarity of their voice embeddings (0.72 and above is one person; numbered in the order voices first spoke); the taps then name them as before. The Transcript is merged by time with each stretch's times and word times shifted by its offset; the Provenance says "Transcribed while recording, in N stretches". Process again transcribes the whole file in one, as before. The live facts are changed under a row lock, since the page and the worker both write them.
  - **The Interpreter's first slice, built in v1.33.0**, text only, with these decisions left to the build and one departure: **one transcription per Turn, not two.** The chapter had Whisper hear a Visitor's Turn twice, once as spoken for the Readback and once translated to English; the build asks the service once, for the words as spoken and the language heard, and asks the engine for the translation in both directions with one fixed prompt (`prompts.TRANSLATE`, register neutral and polite, nothing added). Half the GPU work per Turn, one code path, and the engine is required anyway. The Session is a Live recording of the style `interpreter` (a Dictation's clock when it has no Case), its Turns are rows of their own (`Turn`, migration 0032: side, typed, start, end, language, heard, translation, state), each Turn's audio is posted by the page on its own the moment the pause ends (a second recorder per Turn beside the one streaming the whole Session), heard on the media queue (the fast lane when it answers, at priority 100, waited for inside the task) and translated on the LLM queue; the side is decided by the language heard when no button was held; a Session left to hear its language settles it at the first non-English Turn, with the row "Language settled"; Stop builds the Transcript from the Turns, one Segment each, with two new Segment fields, `language` and `translation`, shown under the words in the viewer and the Word export. The Interpreter is a door on the New recording page rather than a fourth radio card. Not yet built from the chapter: the Voices and their four settings; the notice's translation for a language other than Spanish (English alone until then); the bilingual side-by-side export layout (the translation sits under each line instead); the shipped **Interpreted conversation** summary template. Pause and the shortest Turn are tuned in the room (three quarters of a second; half a second).
  - **From the maintainer, on the first Session (v1.33.1):** "it's only saying thanks gracias, no matter what I say in spanish" and "this needs to be very intuitive, no confusion on who should be speaking". Built in v1.34.0, amending the chapter's turn-taking: **whose Turn it is is never guessed.** One of the two buttons is always lit (Staff at the start); the person taps theirs and speaks; the lit column says "Speak now" in its own language; a Turn ends at the pause and carries its side, so the service is told the language every time and the language-decides-the-side rule of the chapter is withdrawn as the default (it remains only for a Turn posted without a side). The Turn's sound is cut from a rolling buffer of raw samples at 16 kHz with half a second of lead-in and sent as WAV, in place of a second recorder started at the onset, which lost the first syllables and gave the recogniser short quiet clips. Whisper's made-up thanks and sign-offs on such clips ("Thank you", "Gracias", and a short list) are refused as nothing heard. A service that is away when a Turn arrives is asked again every five seconds, four times, before the Turn is written off. The Turn-taking setting's choices are now Tap and Hold, Tap by default.
  - **From the maintainer's first Teams call (v1.35.1), three amendments in v1.36.0:** the call style no longer separates voices within a side by default (each side's channel is one speaker: "This side", "The other side"), with a **Tell the speakers apart** tick under More options that follows the style (on for a meeting, off for a dictation and a call) and may be changed; a speaker rename or merge in the viewer is remembered on the Transcript (`speaker_changes`, migration 0034) and **Undo** puts exactly the moved Segments back, newest change first (Phase 1's viewer chapter gains the same amendment); and a Recording may be **renamed** after the fact by the people who work in it, the file name kept (the row "Recording renamed", carrying no title).
  - **The microphone check, v1.35.1**, from the maintainer's report of a flat level bar with the taskbar showing the microphone in use, after a Remote Desktop session: the Record page gains a microphone chooser (the browser's list, remembered per computer), a live level before Record, and a warning when nothing reaches the app for five seconds before, or eight seconds into, a recording; "Remote Audio" is named when seen. No setting; the page decides nothing for the person.
  - **Withdrawn in v1.35.0.** The maintainer deferred the Interpreter after the first live sessions (chapter 5). Migration 0033 removes the Turn table and the two Segment fields; the media worker leaves the service's network; the page, the settings, and the door go. The amendments above stand as the corrected design.
