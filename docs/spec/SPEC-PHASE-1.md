...
**Toggles.** AI assistant (On or Off; starts Off; Off hides Summary, Chat, and Suggest names everywhere). Chat, Summary, and Speaker suggestions (each On or Off). "Let the model think before answering" (default Off; On sends `enable_thinking: true` on every call and doubles every time limit: Chat 4 minutes, Summary 10 minutes, suggestions 6 minutes). Sampling, answer caps, and time limits are code, not settings.
