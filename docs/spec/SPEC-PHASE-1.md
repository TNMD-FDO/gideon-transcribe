...
- **2026-09-26, v1.92.0.** From the maintainer's walk of the install as a newcomer (the install review of 2026-09-26). The install guide gains the ladder (what a server has and what that opens, keyed on the card's memory, with the download sizes and the host figures, and drawn as `docs/install-ladder.svg`), the quickest install, a Step 0 that prepares Ubuntu (Docker, the driver, the container toolkit with CDI, from the makers' pages recorded in `docs/research/ubuntu-preparation.md`), the upgrade paths under "Add a piece later", and `./transcribe bring-up`. `./transcribe install` prints the ladder with this server's row after the prerequisites, checks the container toolkit's CDI list beside the driver, takes one card by Enter and several by number, asks the engine question in every install (a shared engine needs no card), says whether the card has room for the fast lane, takes the first letter of any fixed answer and asks again on no match, and offers to do the rest itself; `./transcribe check` notes a card with room for a larger batch size than it is set to; the install writes `CARD_MEMORY_GB`, and the upgrade writes it once for an install from before. Chapter 14's Installation page gains "What the card opens": the bar and the two lines from that value; the Pieces card's off rows say what it would take. The README opens with "What to buy". ADR 0015 stands; no glossary word is added, the shapes being keyed on the card's memory.
...
**Sizes.** The app's longest Recording is 6 hours. Rendered as lines with times and Speaker labels, that is about 100,000 tokens at the outside, so the whole Transcript goes into every call, and Phase 1 has no chunking and no map-reduce. Chat history adds at most 16,000 tokens and the largest answer cap is 3,500 tokens (2,500 before v1.53.0), so every call fits the Local engine's window and the Shared engine's with room to spare. The research's map-reduce stays on file for Case-wide work (see Carried for Phase 2).
...
```
Write this summary as a memo a member of staff hands to an attorney: what the recording is, who took part, what happened, and what was said that matters, so the reader knows the situation without listening to it. Third person, past tense, plain words. Group what happened by subject, never minute by minute, and never retell the recording line by line. Give a time as [hh:mm:ss] only where a reader would want to check the recording, after the sentence it supports. These parts, in this order, using these headings.
Summary: one paragraph a reader in a hurry could stop at: what kind of recording this is, who took part, what it was about, what happened, and how it ended.
People: each person who speaks or is spoken of, one line each, with how the recording identifies them and where: a name only as the words give it, a role only as the words give it, and otherwise the transcript's own label for the speaker. Never infer a name or a role from what someone does.
What happened: the substance in a few paragraphs, each on one subject: what was done and what was said, in your own words, quoting only where the words themselves matter.
Statements that matter: exact quotes that carry weight, each with the speaker as the transcript labels them and the time. Quote exactly; never paraphrase inside quotation marks.
...
| Answer caps | Chat 1,500 tokens (about 1,000 words); Summary 800, 1,600, or 3,500 tokens for Short, Standard, and Detailed (600, 1,200, 2,500 before v1.53.0); suggestions 1,000 tokens. A cap hit is shown as it is, with the line "The answer was cut short." |
...
- From the maintainer, on the v1.52.2 build, to "Summary" and "The templates" (v1.53.0; Phase 4 chapter 7): the Standard summary is a memo to the attorney (Summary, People, What happened by subject, Statements that matter, Names, places, and dates, Unclear parts; a name or a role only as the words give it), the wording above being the v1.53.0 one; the answer caps start at 800, 1,600 and 3,500; and a built-in template the office has not edited follows each release's shipped wording by itself.
