...
The WhisperX service returns, on request, one vector per Speaker from its diarization model (`docs/whisperx-api.md`, `return_speaker_embeddings`). The app asks for them today for the stretches of a Live recording and matches labels across stretches by a same-voice threshold (Phase 3, the Live recording chapter), then discards them. Nothing is kept. From chapter 4 (v1.80.0) the vectors are pyannote's alone: the Nemotron diarizer, the default, returns spans and no embeddings, so voice prints will need an embedding step of their own before this chapter can come into force.
...
- **2026-09-25, v1.89.0.** Chapter 4's greyed speaker-count hint says "The diarizer counts the speakers itself; the hint is not used", without naming the diarizer on the page; greyed with the reason, never hidden, as before.

- **2026-09-24 (v1.80.0).** Chapter 4 built, with these decisions left to the build taken: the diarizer image is the WhisperX image's base (the same CUDA 12.8 digest) with NeMo from the pinned source archive on torch 2.8.0, its size to be read at the benchmark gate; the model card's offline chunking values are the service's `.env` defaults (`DIARIZER_*`); the diarizer's version reaches `settings_used` as the model's pinned revision (`diarizer_version`) and NeMo's reported version (`diarizer_nemo`); the Upload page's hint greys under `nemotron`, with a muted line beside it; the refusals are `hint_not_supported` and `embeddings_not_supported` (400) and a Nemotron job while the container is down is `diarizer_unavailable` (503); the pull's words for a skipped gated model are in `pull.py`. One thing not built as written: the Panel does not grey the `pyannote` choice when its weights are missing (the Panel's choice fields have no per-choice greying), so the Status page's WhisperX block lists which diarizers are installed and a pyannote job without the weights is refused by the service with `model_unavailable`, which the recording's failure shows.
