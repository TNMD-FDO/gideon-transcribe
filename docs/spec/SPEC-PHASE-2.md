...
- From the maintainer, on the v1.22.1 build, to the Cases page: **one door.** Clicking a Case anywhere on its row opens the Case page; the Cases page has no details pane and no second "Case page" link. The earlier shape, Open jumping to the newest Recording in the viewer with the Case page behind a quieter link, put people in a player when they expected the case. The viewer shows the Case's name beside its back arrow, which returns to the Case page, and keeps the rail of the Case for moving between its Recordings.
- **Build decisions, v1.20.0 (the items left to the build, and what the code settled):**
  - **The Share table** is `core.sharing.Share` (case, person, added by, added on, last opened; one row per person per Case), migration 0024. Phase 1 had not made it: it arrived with the build.
  - **One set of gates.** `Case.member(user)` says owner, collaborator, or nothing; `Case.role_of(user)` adds admin; `Case.may_be_run_by(user)` is the owner and Admins. For a Recording, `cases.standing(recording, user)` says "own" (the uploader or a member of its Case), "admin" (an Admin looking in, audited), or nothing; `cases.may_throw_away(recording, user)` is the uploader, the Case's owner, and Admins; `cases.affected_by(recording, user)` names the Case's owner, else the uploader, for the row. Every Recording-level page asks these rather than "did you upload this". The media gate does the same, which also lets a reassigned Case's new owner play what somebody else uploaded, a gap Reassign had left.
  - **The Share dialog's words** are fixed in `sharing.WORDS`: "They will be able to do everything in this case except share, rename, or delete it: add recordings, correct transcripts, name speakers, ask for summaries and chats, and save clips." and "Recordings they add count against your storage space." The dialog is one prompt: the words, then a field over a list of the people who may be picked.
  - **The account search** matches the username exactly, else the display name exactly, else one person whose display name or username contains what was typed; two matches is a question back, never a guess. Offered: Active accounts with a first sign-in, minus the owner, the existing Collaborators, and Local admins.
  - **The over-quota message** for an upload into somebody else's full Case keeps the reason class `quota_exceeded` and reads "This batch would go over <owner>'s storage space (...), which recordings added to their case count against." The sidecar's pre-create hook finds the Recording's Case through the upload's metadata and tests that owner's space.
  - **The marks**: a plain pill "Shared by <owner>" on the Cases page row, a green "New" pill until the first opening (the Share's last-opened is empty), and an amber pill with the status word (deactivated, blocked) on a greyed Share row of the "Shared with" panel; the picker and the Upload page's Add to case say "(shared by <owner>, N GB left)".
  - **The cause on "Recording deleted"** when the Case's owner deletes a Recording a Collaborator added is `case owner`; a Collaborator deleting their own writes `owner`, as any uploader does.
  - **Transfer** is built here, from the Case page, for the owner only, through the same list of colleagues. It writes "case reassigned" with `cause: transfer` and the new owner's username, and the old owner's Share writes "Share granted" with `cause: transfer` beside it; a Share the new owner held ends, since an owner needs none. The old owner may leave through "Leave this case", which is a Collaborator removing their own Share ("Share revoked").
  - **A Collaborator may remove their own Share** and nobody else's; the owner and Admins remove any.
  - **The "case shared with you" and "case handed to you" mails** wait for the Email chapter: `sharing` logs that one would go and sends nothing. The Retention digest's shape (`retention.digests_for_tonight`) already carries a line per Collaborator under the owner's name, for that chapter to send.
  - **Nothing carried by the Share row is on the never-logged list**; the rows carry usernames and the Case's snapshot label, as fixed.
...
- From the v1.11.0 build, which shipped this chapter ahead of the Cases release because Suggest names needed the People (`docs/research/speaker-suggestions.md`), the decisions left to it and three departures the maintainer accepted:
  ...
  - **The rename box** lists the Case's People through the browser's own suggestion list under the field (a `datalist`), each as "Name (Role), in N recordings"; there is no "New person: <typed>" entry, since typing any other name makes one, and the note under the box says so. In the Workspace the box is the browser's prompt, as in Phase 1. **Amended in v1.36.1:** the `datalist` is withdrawn, because the browser filters it by the field's text, and the field holds the Speaker's current name, so nothing was offered until the field was cleared (the maintainer: "the dropdown isn't working"). The People are listed in the open under the field, one button each (name, Role badge, "in N recordings"); a click names the Speaker at once, typing narrows the list to the names containing the text and shows "Nobody in this case matches; Save makes a new person" when none does, and a Case with no People says so. The chapter's rule stands: a name that matches a Person joins it, any other name makes one.
...
- **An Incident's cameras are one item** (v1.87.0, Phase 8 chapter 9). The synced cameras of an Incident among the Recordings a question reads go into a Reading as one item, the Incident's record: each camera's Digest, or its words when it has no Digest, merged in time order by the cameras' clock, every line cited as `[Recording n, hh:mm:ss]` on that Recording's own clock, the cameras' own Transcripts and Digests not sent as well. When the record would not fit a Reading, the longest unpinned camera is read by its words alone, then the next, as the Sitting does; when it cannot fit even so, the cameras stay separate items as before. Each camera keeps its header line; an answer's opening line names a camera read from its words as it does any video read without its picture. A part, when there are parts, is told which Recordings it holds and that the other parts hold the rest, so it never says the case lacks what its share does not contain; the combining call writes one answer in time order, treats a difference between parts as coverage rather than disagreement, and names a disagreement only where two parts describe the same moment differently.
...
The AI assistant writes `[Recording 3, 00:12:45]`. The app matches the Recording number to the question's header lines and the time to a Segment start of that Recording (the Phase 1 match rule) and shows the Citation as the Recording's title and time, "Jail call 2026-03-04 at 00:12:45", a link that opens that Recording in the viewer at that moment. A Citation the app cannot match stays plain text, as in Phase 1. (Amended 2026-09-24, v1.82.0, from the maintainer's finding that what the camera showed no longer linked: a time that is not a Segment start but lies inside the Recording's length is matched to that second itself, since the Digest's picture moments have times of their own, and the pill's hover then shows the Digest's line marked seen for that moment as "Seen: ..."; and a short `[hh:mm:ss]` in a paragraph that has named "Recording n" before it, the shape the engine falls into once a sentence has named the recording, is read as `[Recording n, hh:mm:ss]`, the nearest name before it, and rewritten to that shape in the saved answer so the page, the export and the combining call see one shape. A time past the Recording's end, a number the question did not read, and a short time in a paragraph that names no recording stay plain text.) A Recording later deleted or moved to another Case leaves its Citations as plain text marked "(recording removed)". In the combining call the Readings' Citations are kept word for word and never invented; the app checks the combined answer's Citations again before display.
...
```
Answer the user's questions using only the transcripts of the recordings in this case and, where a recording has one, its record of what the camera showed. If the answer is not in them, say so in one sentence and do not guess. Quote the transcripts when it helps. For every statement you rely on, give the recording and the time as [Recording 3, 00:12:45], copied from the line it appears on, and never make one up. When several recordings bear on the question, say which says what. Keep answers short unless the user asks for detail. If a question asks for legal advice, an opinion on guilt or credibility, or anything outside these transcripts, reply: I can only answer from the transcripts in this case. When a recording is a camera of an incident whose clock you were given and the question asks when something happened, give the time of day by the cameras' clock beside the reference; asked what another camera showed at that moment, read that camera at the same time of day, and say when it had not started or had stopped.
```
...
```
The same question was answered separately over several parts of one case's recordings, each part reading different recordings. Write one answer from these part answers, in time order by the cameras' clock where the parts give one. A difference between parts is what different recordings covered, not a disagreement: name a disagreement only where two parts describe the same moment and say different things. Keep every [Recording n, hh:mm:ss] reference exactly as written and add none. Leave out a part's report that its recordings do not mention something, unless no part found an answer; then say the transcripts do not answer it.
```
...
| Time limits | 15 minutes for the whole question; each engine call keeps the Chat rule of two minutes, plus one second for every 500 tokens the call sends (v1.87.0, every feature's calls alike); both doubled when "Let the model think before answering" is On |
...
- **2026-09-26, v1.90.2.** On the office's copy the case chat, asked when rights were read, added a camera's start on the clock to a line's own time and got it wrong by twenty minutes (21:03:52 for 20:43:52). The record lines the case chat reads for an incident (v1.87.0) now open with the time of day by the cameras' clock in parentheses, before the reference in square brackets; the record's head and the Case chat template say to copy it as it stands and never to work out a time from a camera's start. About five tokens a line more in the Reading.
- **2026-09-26, v1.90.1.** The v1.90.0 wording had given the model a refusal sentence to quote, and on the office's copy it opened a two-thousand-word answer with it and then listed every line one camera recorded, each marked "Statement", instead of the questions asked. The Case chat template is re-shipped again: answer the question that was asked and nothing beside it (the questions and their answers, not every line spoken; the moments, not the transcript); give the part the transcripts hold and say in one sentence what is missing; never open with a disclaimer; refuse only advice and opinion, in one sentence. The AI assistant call audit row for a case chat turn records whether the answer was cut at the cap.
- **2026-09-25, v1.90.0.** From the maintainer's walk of the outputs: a question of fact put to the case chat about rights, questioning, searches or force was refused with "I can only answer from the transcripts in this case". The Case chat template is re-shipped: a question about what was said or done is answered however it is worded; the refusal is kept for advice, opinion and facts the transcripts lack, and then says what they do hold.
- **2026-09-25, v1.87.3.** From the maintainer's timeline on the office's copy, where the model cited one recording at 00:00:00 a hundred and ninety times down its list: the Case chat answer format says that every line of a list carries the reference of the line it rests on and that a list never repeats one reference for several lines.
- **2026-09-25, v1.87.2.** At the maintainer's word after the v1.87.1 timeline (274 lines, one line in the camera's own words): in a timeline or a list, a line that rests on the picture begins with "Camera:", so a reader can tell a description from a spoken fact; one sentence more in the record rules.
- **2026-09-25, v1.87.1.** From the maintainer's finding that a timeline asked of the case chat left the cameras out: a call that reads a Digest is now told the record's marks and that the picture belongs in an account of what happened (the record rules, app plumbing after the camera rules, in the case chat and a recording's chat alike), and the Case chat template is re-shipped naming the record of what the camera showed beside the transcripts. The template block above and the Combining block now show the wording as shipped (the Combining block had not followed v1.87.0).
- **2026-09-25, v1.87.0.** From the maintainer's finding on the office's copy that a question over twelve cameras was split into three Readings, each answering as if its cameras were the whole case, and the combining call dressed the differences up as conflicts; and that the third Reading ran into the fixed per-call limit. The Readings rule gains "An Incident's cameras are one item" (the Sitting of Phase 8 chapter 9, in the Case Chat), the part line and the stitching combine; the time limits row gains the allowance for what a call sends. Decided by the build: the allowance is one second per 500 tokens, a floor measured on the office's engine, kept in code rather than as a setting; a camera in two Incidents is read with the first; an Incident with one camera among the Recordings read is not an item.
...
- 2026-09-24 (v1.82.0): Citations amended in place, a cited time inside the Recording's length links whether or not a line starts there, and a short time after the recording's name takes the full shape.
...
- From the maintainer, on the v1.37.0 build, to "One Reading", "Two at a time", and the caps: **the Reading sizes and the Case Chat's budgets become settings** on the AI assistant page, each with the chapter's value as the default and Reset to default beside it: Reading size (100,000 tokens), Readings at once (2), Case chat answer cap for each part (1,500) and combined (2,000), and the Case chat question time limit (15 minutes). The engine window for "would not fit even alone" is the Phase 1 Engine window setting. The packing rule, whole Transcripts never cut, is unchanged.
...
- **Build decisions, v1.21.0 (the items left to the build, and what the code settled):**
  - **The mailer** is `core/mail.py`: smtplib directly rather than Django's backend, so the three SMTP steps are taken one at a time and the relay's own reply to the last is what the test message shows. Configured means `SMTP_HOST` and `MAIL_FROM` both set. `SMTP_STARTTLS=auto` upgrades when the relay offers STARTTLS and refuses to send a password without it (`smtp_auth_failed`); `always` fails as `smtp_unreachable` when the relay offers none; `never` sends plain.
  - **Marked automatic** with `Auto-Submitted: auto-generated` (RFC 3834), `X-Auto-Response-Suppress: All` (Exchange), and `Precedence: bulk`; `From` is `Gideon Transcribe <MAIL_FROM>` through `formataddr`; `Reply-To` the Operator address.
  - **The queue**: one `send_email` task per message on the `default` queue; a failed try re-queues itself with `schedule_in` 5 and then 20 minutes, so the three tries fall at once, +5, +25; the third failure writes "Email failed" with the reason class and tries; a row is written per message, never per try. The Status row `MailStatus` (migration 0025) carries the last sent and last failure.
  - **Rendering**: the subject and body are the settings, falling back to the chapter's default when an Admin has emptied one; an unknown placeholder in a template is left as it was rather than failing the message; an empty block collapses its blank paragraph (no run of blank lines survives). Apply refuses a placeholder the kind does not have and does not refuse an omitted one.
  - **The digest when `{cases}` is empty** and only `{shared}` has lines: the block prints "- (none of your own)" under the editable introducing line rather than leaving it hanging. Digest lines read lowercase "deletes in N days unless used" as the chapter's sample; the Cases page keeps its capital.
  - **`{failed_list}`** is one line per failed Recording, "- <title>: <reason in plain words>" (the media step's failure message or the Job's reason class in words); **`{time}`** is "06 September 2026 14:05" in office time; a Batch cancelled whole, or with no Recordings left, sends nothing; a Process again's Batch never carries the tick.
  - **The Upload tick** is stored on the Batch (`email_when_done`) and remembered from the person's last upload Batch; it is offered only while "Email notifications" and "Batch finished emails" are both On and mail is configured (so Off on the master switch hides it too), and greyed with "No email address on your account; ask IT" for a person with none. `mail_sent_at` on the Batch is the once-only mark. The end of a Batch is noticed after every Job completes or fails and after every media failure, not by a timer.
  - **The test message** sends within the request, one try, no queue, so the relay's reply is on the page; its row records the recipient address and the reply, or the reason class. `./transcribe check` sends the same message to the Operator address through `python manage.py mail_check test`.
  - **The Operator message about Cases whose owner has left** reads "These cases belong to people who can no longer sign in, and they delete unless somebody uses them. Reassign or Keep them from the Cases page's "Owner deactivated" and "Expiring" filters." followed by "- <Case> (owner: <name>): deletes in N days unless used" per Case and the Cases page's link with both filters set. The four backup messages' wordings are in `mail.backup_failed`, `backup_overdue`, `drill_failed`, `drill_overdue`, `restore_completed`: plain text, no content, no names, each saying what to run on the server.
  - **The `check` lines** read "the relay <host> accepted a test message to <address>: <reply>" and "every one of the Sign-in group's N members has a mail value", the latter amber with the usernames when some lack one; "SMTP_HOST is empty, so the app sends no mail" as a note. The install's questions are in `ask_the_mail`, after the backup's, with the blank-host warning in amber; `./transcribe install-mail` asks them again.
  - **The Reassign mail** is sent per Case from `_reassign`, naming the Admin; Transfer's from `sharing.transfer`, naming the old owner. The share mail is sent only when the Share is newly made.
  - **Greying by the environment**: `Definition.needs_mail` beside `needs`; `settings_store.greyed_because()` is the one answer the panel reads, and `edit` never reads a greyed row back, so a closed toggle is not taken as Off.
  - **A Local admin's address** is typed on Create Local admin and set on the row (action `email`), writing "Email address updated" with the Admin as actor.
...
- From the v1.19.0 build, which shipped this chapter ahead of the Cases release, the decisions it left to the build and two departures:
  - **The two backup secrets are never mounted into the live stack.** The chapter had the app's preflight check them; the build checks them on the host in `./transcribe backup` (`target_unreachable` when one is empty) and the app's `backup_preflight` checks only the room and the target, so no container of the live stack ever holds the repository password or the store's key. They are the `backup` service's Compose secrets alone, bind-mounted with the host files' own owner and mode (the app's account, 0400), which is what ssh wants; and since the container runs as that account and Alpine's passwd file does not know it, `./transcribe` writes a one-line passwd file into `backup/.passwd` before every restic run and mounts it, or ssh refuses to start with "No user exists for uid". The configuration is staged into `backup/config/` by `./transcribe backup` just before the Snapshot, a copy owned by the app's account with `backup_password` and `backup_ssh_key` left out, and that folder is what the container mounts: the Install home's files belong to whoever installed and are not readable by the container, and mounting `secrets/` whole would have carried the two into the repository. The Installation page's "whether the two backup secrets are set" row is therefore "Backup last run" from `backup/last-run.json`.
  ...
  - **The Restore drill's project** is `transcribe-drill` from `compose.yaml` with `compose.drill.yaml`: Postgres and the app alone on the `transcribe-drill` network, every other service behind a profile the drill never asks for, directory sign-in off, no mail, and the restored data under the drill folder. Its Postgres keeps the `transcribe-postgres` alias the app connects by, and its Postgres folder is root's at 0755 (Postgres runs as its own user and must be able to enter it). After the dump loads, the drill runs `ensure_roles` before `migrate`, as the web container does at every start: a dump carries the database and never the cluster's roles. Management commands in the throwaway project go through the image's entrypoint by name (`run --rm app migrate`), never as `python manage.py`. A manifest that is missing or names no tables fails the check: a drill that restored nothing must not pass. The drill folder is emptied by root (`find -mindepth 1 -exec rm -rf`), never by a shell glob under sudo, and emptied before the drill as well as after it.
