...
> UNRESOLVED: whether a Collaborator's Move to case of their own Done Recording into a shared Case can be refused for room. "Case folder model on the mount" says "a Move to case never changes the count, so a move never fails", written for a user's own Cases. "Sharing model" says a Collaborator's "Move to case of their own Done Recordings" is allowed and "counts against the owner's quota", and says only of uploads that a full quota refuses them. Neither says whether such a move is refused when the owner's quota is full, and no amendment settles it.
