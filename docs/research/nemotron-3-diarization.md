...
**The reference, checked by ear.** The maintainer doubted the reference, and the audit log bore that out: the 164 corrections were accepted in two bursts of a minute each (104 at 23:20, 60 at 23:41 on 2026-09-14), Accept all, never adjudicated. So the twenty stretches where Nemotron disagreed with the page (the ones the figures above count against it) were put to the maintainer's ears on 2026-09-24, one question each: is the speaker the page shows the one really talking? The answers: **No 13, cannot tell 7, Yes 0.** Of the thirteen decided, Nemotron's reading matched the ear in eleven (three "an extra voice" where the page named one of the five; four Speaker 3 and one Speaker 5 where the page named another; three "nobody, noise" where the page had words under a name), was partly right in one (an unlisted voice where it named a fifth) and unclear in one. pyannote's original label matched the ear in three of the thirteen (the three the Speaker check had moved to the wrong speaker), so those three corrections made the transcript worse. The stretches where the office had moved a line (section B) were not judged; the maintainer's view is that those changes were accepted without listening and say nothing.

**What that means for the table.** The 8.8 % against 32 % is a comparison against a reference that was wrong wherever it was checked, so it measures agreement with pyannote-plus-Accept-all, not accuracy. On the disputed third of the words, Nemotron was right and the page wrong in every decidable case. A true error rate for either system cannot be read from this recording as it stands; what can be said is that Nemotron found voices the page had merged into named speakers (radio and background talk), heard silence where the page had words, and agreed with the ear on speaker identity where the page did not. The over-splitting the first figures showed was, at least in part, real extra voices.

**What it decided.** Not a pivot on the evidence, and not a dismissal either: the measurement was void. Three things follow. First, a reference has to be made by ear before either diarizer can be scored: a stretch of a recording labelled by a person, word by word, with no machine label in view; the scoring script takes that in place of the page's names. Second, the Speaker check's Accept all is dangerous on a recording like this one (a night-time body camera with radio traffic): three of the four judged corrections moved a line from the right speaker to a wrong one, and the office should accept the check's corrections one at a time, listening, or not at all; that is a finding for Phase 5 chapter 3 regardless of Nemotron. Third, Nemotron 3 is worth a proper trial: fast, ungated, and right by ear where the page was wrong, against the costs already listed (no count hint, no embeddings, a git commit for a pin until NVIDIA releases). The trial the maintainer can choose: a second diarizer choice in the service, run on new recordings beside pyannote for a few weeks, with the Speakers page showing both splits so the ear judges in normal use; the chapter for that would be written first. The Sortformer paragraph in `speaker-pipeline-review.md` is corrected on its ceiling, and its conclusion that the app runs the right model is now an open question.

**The stack, settled for the build.** A third probe image installed torch 2.8.0 and torchaudio 2.8.0 with CUDA 12.8 first (the WhisperX stack's own wheels) and NeMo from the same commit on top: NeMo accepted them (it reports 3.1.0 at `cf724ac3`; transformers 5.17.0, lightning 2.4.0, numpy 2.5.3), the model loaded, and the spans were byte for byte the same as the CUDA 13 run, in 5.0 seconds with 2.3 GB of card memory. So the diarizer container runs on the same torch and CUDA as the WhisperX image and the package's driver requirement does not rise (ADR 0014). That image was 14.6 GB unoptimised; the build's is to be lean.
