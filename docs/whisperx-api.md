...
The WhisperX service is the independent transcription engine of Gideon Transcribe. It transcribes, aligns, and diarizes one audio file per request on the GPU, using a fresh, pinned build of WhisperX and nothing reused from any older app. It is not part of the app: it has its own folder in the repository, `whisperx-service/`, with its own Dockerfile, `compose.yaml`, `.env.example`, `models.yaml`, version number, and README, and it runs without the app. Any application that holds a token for it is a Consumer and uses the same contract; Gideon Transcribe is the first Consumer. The service runs exactly one job at a time across every Consumer, in arrival order, and that serial behaviour is part of its published contract, not an internal detail.
...
| `model` | string | `large-v3`, `large-v3-turbo` | `large-v3` | The allow-list. Anything else is `400`. |
