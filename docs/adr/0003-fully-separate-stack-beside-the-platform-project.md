...
**Noted 2026-09-09, when the coupling went live.** The office's install joined the platform's engine that day: `./transcribe engine` with the platform's network, address, model name and key, handed over outside both repositories, and the local engine profile off for good. The contract between the two (who announces an engine restart, how a key rotation arrives and that it needs no container restart here) is in `docs/box-ledger.md`, open item 5 and the two projects' log lines of 2026-09-09.
