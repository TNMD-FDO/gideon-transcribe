...
- **Summary.** **New summary** asks what to concentrate on (optional) and how long: Short, Standard or Detailed. If your office has more than one template, you choose one; a recording in a case with a type (a jail call, a body camera recording, an interview, a phone call, a hearing) starts on the template made for that type, and the dialog says so, with the others still on offer. On a video with Moments on, the button reads **Summarise this video**: a video without a type starts on the **Video summary**, which is one account of the events, an executive summary first and then what happened in order, written from the words and the camera's descriptions together (a body camera recording keeps its own template, whose timeline is one account too). The summary does not say which sentence came from the words and which from the picture; it never names a person from the picture alone, never calls a thing more than the description saw, and never states a legal conclusion. The dialog's line says whether the video is prepared, and what preparing it first will take. A summary lists what happened with a time on each point; a time shown as a link is a **citation** that plays the recording from that moment, with a small camera mark when it points at a described stretch. **Regenerate** writes it again with the same choices; **Export to Word** keeps it; **Delete** removes it. Summaries live as long as the recording does, so export what you want to keep.
...
- **Videos are prepared.** On a video, when your office has turned Moments on, the assistant looks at the picture as well as the words. Nothing to press: as soon as the transcript is done the app scans the video for where the picture and the sound change, describes each stretch once (none longer than fifteen seconds unless your office chose otherwise), reads the date, time and camera id many body cameras burn into the top of the picture, and condenses the words and the descriptions into a digest the summary and the chat are written from. The batch page shows it happening, "preparing 12 of 60, about 18 minutes", and says when a batch's videos are all prepared; if you ticked **Email me when this batch finishes**, the message waits for that and says how it went. A case's recordings list says **Preparing** in its State column while that runs (the transcript can be read; the summary and the chat wait), has a **Prepared** column with the count, and **Prepare them now** for any video that is not. Ask for a summary or a chat answer before a video is prepared and it is prepared first, the card saying so and how long. The descriptions themselves print in a summary's and a transcript's Word export under **What the camera showed**; the transcript itself carries nothing about the camera.
