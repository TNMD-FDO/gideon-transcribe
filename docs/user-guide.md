...
When your office has Incidents on, a case's page carries an **Incidents** strip under the case name. An incident is the cameras that ran at the same time, the body cameras of every officer at a stop and the in-car camera of each unit, laid on one clock so they play in step. The app reads the date and time each camera burns into its picture as its playback copy lands, and when two or more of a case's videos overlap on their clocks the strip offers them: "8 videos ran at the same time on 7 June 2025. Make them an incident?" **Make it** makes it and opens it; **Choose myself** opens the same dialog as **New incident** in the header, with those videos ticked, for a group of your own; **Not these** puts the offer away. A video uploaded later that ran during an incident is offered to it the same way. The recordings list shows each video's **Clock in the picture** and which **Incident** it is in.

**Open the incident** opens the incident's own page. Up to six cameras play together on the wall (your Admin sets how many); the rest wait beside it as parked tiles, with **Swap in**. One transport plays them all, the big clock reads the time of day by the cameras' clocks, and **Sound from** picks the one camera you hear. A camera that has not started yet says "Starts in 0:47" and starts when its time comes; one that has ended says so. Under each picture, the words being said on that camera and, on a video enriched with vision, what its camera showed. Across the width, a strip with a lane per camera: click anywhere to play every camera from there, drag a camera's bar to place it by hand, and zoom to ten minutes or two.

Each camera carries a pill saying how it was placed on the clock: **From its clock, checked** (the best the app can do by itself; two cameras placed this way agree to a second or two, because camera clocks drift), **From its clock, unchecked**, **Matched by sound**, **From its file, unchecked**, **Placed by hand**, or **Not placed**. The **Cameras** tab is where a camera without a clock is placed: **Match the sound** compares its sound with a placed camera's and lines them up when the two were within earshot (about a minute, nothing goes to the AI engine); **From its file** takes the time the camera wrote into the file, unchecked; the nudge buttons and **Type a time** place it by hand. The same tab adds cameras, removes one (the recording stays in the case), renames the incident, or deletes it (only its placements go; never a recording). **Details** says who made the incident, where its clock came from, and each camera's stamp.

A recording in an incident has **All cameras** in its viewer's head, which opens the incident at the moment being played, and a case chat's citation on such a recording carries an **all cameras** link that does the same. An incident belongs to its case: everyone the case is shared with can work in it, it counts as using the case, and it goes with the case to the Recycle bin and back.
