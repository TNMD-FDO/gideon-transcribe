...
- **Speaker check.** When your office has it on, the assistant reads a transcript whose speakers were told apart and looks for lines the words show were given to the wrong voice: a question and its answer under one speaker, someone addressed by name answering under the asker's name. It only ever proposes a move to a speaker already in the transcript, never invents or merges a speaker, and never changes a word. The proposals wait as **Suggested corrections** on the Speakers page; the recording page's speaker strip shows how many. It runs by itself as each transcript lands, or overnight if your office prefers, and **Check the speakers** on the Speakers page runs it whenever you like.
