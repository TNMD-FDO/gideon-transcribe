...
- **Moments.** On a video recording, when your office has turned Moments on, the assistant can say what the camera showed at a time the words only point at: "look at that", "there's the bag", a handover, a scuffle. Press the small **camera** button on any transcript line, or **Describe this moment** in the Moments tab for wherever the player is; a box opens. Leave it empty and the app cuts about ten seconds of the picture around that time, shows it to the assistant with the words spoken in it, and a short description appears under the line, marked **Camera**, and in the Moments tab; the clip itself is not kept. Or type a question, "is that a gun on the seat?", "what is in his left hand?", and the assistant looks at a few close frames at the camera's own detail and answers in three parts: what is **visible**, what that is **consistent with**, and what **cannot be told** from the frames and why. It will not tell you a dark shape is a gun when the picture cannot settle it; that is the point. **Find moments**, under **Suggested moments** in the Moments tab, reads the whole transcript once and suggests the lines where the picture would add a fact, each with a reason: an object named or handled, a command that implies an action, an action narrated, a pointing phrase, a sudden change. Where your office has turned it on, it also scans the picture for sharp changes and the sound for raised voices or a bang. Each suggestion shows its time, its kind, its reason and where it came from, with **Describe** and **Dismiss**, and its line carries a dashed **Camera?** pill; nothing is described until you ask. **Describe the whole recording**, at the head of the Moments tab, makes a moment every so often through the whole video (every minute by default) and describes them one after another, skipping times already described; the tab counts them as they land. On a video, **New summary** offers a tick, **Describe the moments first**: ticked, the summary does that before it writes, so it can say what was seen as well as what was said, citing the camera's moments and saying "the camera shows"; its Word export ends with **What the camera showed**, every described moment with its time, so a reader sees what it drew on. A description is what a model saw, never the transcript: it names people by their clothing and position, not by name, says "a small bag" rather than what the bag holds, and "not visible" when it cannot tell; check it against the picture before relying on it. **Edit** puts your own words in its place and marks it edited, **Again** asks once more, **Delete** removes it, and **Make a clip** marks the moment's span in the Clips tool with the description as the note, so the clip you cut for a motion or a hearing carries what the camera showed; with captions burned, the camera line appears in the picture too. Camera lines go into the Word and plain-text exports, marked and with a legend, and, when your office has left **Moments in answers** on, into Summary and Chat, labelled, so a summary can say what was seen as well as what was said.
