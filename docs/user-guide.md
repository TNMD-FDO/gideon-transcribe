...
**Follow** keeps the current segment in view as the recording plays: the transcript moves only when the line being spoken has left the middle of the column, so it neither creeps nor jumps. It pauses when you scroll the transcript yourself, and **Resume** or the F key puts it back.
...
**Manage speakers**, beside the list, opens the Speakers page: a page of its own whose one job is saying who each speaker is. Down the left is a card for each speaker, in the order they first spoke, with how many lines they have and how long they talk, three **sample** buttons that each play one of their lines on its own so you can hear the voice, **rename**, and **same person as**. An unnamed speaker's card has a box that asks "Who is this?"; inside a case the case's people are offered first, with their roles, and a new name can take a role from the list as you make it. A card that says "Never speaks while Speaker 1 does" is the page pointing at a voice the engine may have split in two, with a button to merge it. On the right the recording plays, and under it are the **lanes**: one row per speaker across the whole recording, with a block wherever they talk. The lanes are the scrub bar: click anywhere to play from there, click a speaker's name to pick their card, and drag one name onto another to merge them. Zoom to two minutes or thirty seconds when a long recording squeezes the blocks. Under the lanes the transcript follows along, showing the picked speaker's lines with the line before and after each for context, or everyone. On the line being spoken, **not this speaker** hands the line to someone else, and the number keys do the same: press 2 and the line becomes speaker 2's. Space plays and pauses, B goes back three seconds, the arrow keys move five. **Suggest names** at the top asks the AI assistant, when your office has it on, and each suggestion lands on its card with Accept and Reject. **Undo** at the top takes back the last change. **Done** returns to the recording at the same moment, and **Open in a window** puts the page in a window of its own that the recording's page follows, for a second monitor.
...
- **Moments.** On a video recording, when your office has turned Moments on, the assistant can say what the camera showed at a time the words only point at: "look at that", "there's the bag", a handover, a scuffle. Press **Describe this moment** in the Moments tab for wherever the player is; a box opens naming the time. Leave it empty and press Describe: the app cuts about ten seconds of the picture around that time, shows it to the assistant with the words spoken in it, and a short description appears under **Described moments** in the tab; the clip itself is not kept. Or type a question, "is that a gun on the seat?", and the assistant looks at a few close frames at the camera's own detail and answers in three parts: what is **visible**, what that is **consistent with**, and what **cannot be told** and why. It will not tell you a dark shape is a gun when the picture cannot settle it; that is the point. **Describe the whole recording** makes the **picture record**: the app scans the picture and the sound once for where they change, cuts the video at those points into stretches that touch and never overlap, none longer than fifteen seconds unless your office chose otherwise, and describes each stretch once, telling the assistant what the stretch before it showed so it says only what is new; a stretch where nothing changed comes back as one line beginning "Unchanged". It says how many descriptions before you confirm, skips stretches already described, and the tab counts them as they land. Nothing about the camera sits in the transcript itself: the descriptions live in the Moments tab, in a summary's and a transcript's Word export under **What the camera showed**, and in what the assistant is told. The first time the record is made, the app also reads the **camera's stamp**, the date, time and camera id many body cameras burn into the top of the picture, from two frames a minute apart so the clock can be checked; Details shows it, and the assistant is told it as the camera's clock, so a summary or an answer can give a time of day. It is copied as printed and never reordered.
