...
Choose the visitor's language, or leave it on **Let the app hear it** and the first thing they say settles it. Press **Start** and talk. The screen splits into two columns, the visitor's on the side facing them (**Swap sides** turns them round). Each time somebody finishes speaking, their words appear in their own column as the app heard them, and the translation appears a second or two later in large type for the other side to read. Two large buttons, **Visitor** and **Staff**, are there for a loud room: hold one while speaking and release when done. **Type instead** sends a name, a number, or an address as words, and the **quick phrases** ("Please wait a moment", "We are arranging an interpreter", and the rest) go to the visitor in their language with one press.
