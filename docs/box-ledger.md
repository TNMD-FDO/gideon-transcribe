...
**Rules version:** 2026-09-09.5 (sections 1 to 11)
...
| | Host RAM |
|---|---|
| GIDEON | its hardware profile's memory table (`models.lock`, spec §7.6; a release value, never changed on the server by hand, like the fractions above): 73 GB today across twelve services, and about 229 GB from the corpus slice (`v0.4.0`: Qdrant 110, OpenSearch 48, the workers 12). Its throwaway projects (the drill on this server, the acceptance VM at 16 GiB) run inside this line and in its windows. |
| Transcribe | 40 GB (its Compose limits, about 39 GB at v1.32.2), with its local engine's 16 GB on loan beside it while that engine runs, as with its card memory. |

- Each project's host memory is its line in the table above. Taking more is a ledger change first, as the cards' rule has it; a limit is set in a release (GIDEON's lock, Transcribe's Compose file), never on the server by hand.
